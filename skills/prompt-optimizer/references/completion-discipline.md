...
## Rule 1 — the done-condition rides the final step
...
The evidence rides the closeout sentence that CLAUDE.md already requires, so this costs
nothing in output. "3 surfaces × 5 files identical" is a measured claim; "everything is in
sync" is not.
...
- Depth arithmetic is fiction: "tree 6" cost 1.0–1.5x "tree 3", never the promised 8x.
  Models treat decomposition depth as a thoroughness dial and ignore the multiplier.
...
scored six cases with Table-1 overrides silently unapplied; the scheduled review emailed
